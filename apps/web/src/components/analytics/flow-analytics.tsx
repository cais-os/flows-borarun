...
import { FlowAnalyticsMap } from "./flow-analytics-map";
...
export function FlowAnalytics() {
  ...
  const fetchFlows = useCallback(async (range: DateRange) => {
    setLoading(true);
    try {
      const res = await fetch(`/api/analytics/flows${buildQueryString(range)}`);
      if (res.ok) {
      ...
      // Ignore fetch errors to avoid blocking the page.
    } finally {
      setLoading(false);
    }
  }, []);

  const fetchFunnel = useCallback(async (flowId: string, range: DateRange) => {
    setFunnelLoading(true);
    try {
      const res = await fetch(`/api/analytics/flows/${flowId}${buildQueryString(range)}`);
      if (res.ok) {
  ...
  const selectedFlow = flows.find((flow) => flow.flowId === selectedFlowId);
      ...
      {loading ? (
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
            <StatCard title="Execucoes" value={funnel?.totalExecutions ?? 0} icon={Play} />
            <StatCard title="Concluidos" value={funnel?.completed ?? 0} icon={CheckCircle2} />
            <StatCard title="Abandonados" value={funnel?.abandoned ?? 0} icon={XCircle} />
            <StatCard
          ...
          <div>
            {funnelLoading ? (
              ...
              <p className="text-sm text-slate-400">Sem dados de execucao ainda.</p>
            ) : (
              <div className="space-y-6">
                {funnel.layoutNodes.length > 0 ? (
                  <FlowAnalyticsMap funnel={funnel} />
                ) : (
                  <p className="text-sm text-slate-400">
                    Este flow ainda nao possui etapas suficientes para montar o mapa.
                  </p>
                )}
                ...
                <div>
                  <div className="mb-3 flex flex-wrap items-end justify-between gap-2">
                    <div>
                      <h3 className="text-sm font-semibold text-slate-700">
                        Funil em ordem de execucao · {funnel.flowName || selectedFlow?.flowName}
                      </h3>
                      <p className="mt-1 text-xs text-slate-500">
                        Leitura linear para comparar rapidamente o volume e a perda
                        entre as etapas.
                      </p>
                    </div>

                    {funnel.totalExecutions === 0 && (
                      <span className="rounded-full bg-slate-100 px-2.5 py-1 text-[11px] font-medium text-slate-500">
                        Ainda sem trafego neste periodo
                      </span>
                    )}
                  </div>
                  ...
                  {funnel.nodes.length === 0 ? (
                    <p className="text-sm text-slate-400">
                      Sem etapas registradas para este flow.
                    </p>
                  ) : (
                    <div className="space-y-1">
                      {funnel.nodes.map((node, index) => {
                        const Icon = NODE_ICONS[node.nodeType] || Zap;
                        const maxVisits = funnel.nodes[0]?.visits || 1;
                        const barWidth =
                          maxVisits > 0
                            ? Math.max(4, Math.round((node.visits / maxVisits) * 100))
                            : 4;
                        ...
                        return (
                          <div key={node.nodeId}>
                            ...
                            {index < funnel.nodes.length - 1 && node.dropOff > 0 && (
